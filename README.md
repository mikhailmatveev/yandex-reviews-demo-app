...
С настройками `.env` по-умолчанию frontend будет доступен по адресу http://127.0.0.1:9000 , backend - http://127.0.0.1:8080.
...
Порт, на котором работает frontend, настраивается через параметр `WEBPACK_DEVSERVER_PORT` в конфиге `.env` (пригодится, когда локальный порт по-умолчанию `9000` уже занят), например:
